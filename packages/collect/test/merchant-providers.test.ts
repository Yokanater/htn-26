...
describe.each(['outfit', 'setup'] as const)('%s provider pipeline', (domain) => {
  it.each(['', '/en-ca'])(
    'combines browser capture, localized %s variants and quoted Baseten facts',
    async (locale) => {
      const progress = vi.fn();
      const browser = vi.fn(async () => ({
        url: `https://store.example${locale}/collections/all`,
        text: 'Products',
        productUrls: [`https://store.example${locale}/products/item`],
      }));

...
Implemented behind `PROVIDER_MODE=browserbase`: bounded Browserbase storefront extraction for editable profile suggestions; real candidate storefront capture for collaborators and competitors; validated storefront imagery for result cards; public-DNS and redirect checks; `robots.txt` policy; domain-restricted sessions; Shopify signals; source excerpts, URLs, and content hashes; three-session global concurrency; a hard five-minute market-research deadline; cancellation that closes active sessions; and session cleanup. Live runs preserve supported partial results and never fall back to fictional candidates. Footwear merchants cannot receive another footwear assortment as a collaborator; a narrow exception permits care/accessory specialists such as socks, insoles, laces, and shoe care.

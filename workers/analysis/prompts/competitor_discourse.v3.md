...
## Competitors (5-10)
- A competitor is a brand a merchant's customer might buy **instead of** the merchant's products (`direct`), to do the same job another way (`substitute`), or via an overlapping audience/channel (`adjacent`).
- **Brands that sell complementary products are not competitors.** Grinders, kettles, filters, drinkware, baked goods and gift makers are collaboration partners even when they appear in the evidence and are relevant. Never list one here, and never list the same brand twice. Use the candidate seeds as your strongest guide to who the competitors are.
- Do not pad the list to reach the minimum. Each entry must be a distinct brand that genuinely competes.
...
- **Customer discourse may cite only `origin="independent"` documents.** A price, shipping threshold or product listing taken from a storefront (`brand_owned`/`merchant_owned`) is a fact for the competitor's `claim`, never support for a theme: if a theme is about price, cite only the forum/review/editorial posts that complain about it, even when a storefront shows the number. Never cite `merchant_owned` or `brand_owned` documents in a theme, and never treat marketing copy as customer sentiment.
